...
def create_nodos_release(gh_release_repo, gh_release_title_postfix, gh_release_target_branch, dry_run_release, skip_nosman_publish, bundle_info):
	...
	for path in artifacts_abspath:
		...
		nosman_args = [f"nodos", "publish", "--path", path, "--name", nodos_package_name, "--version", f"{major}.{minor}.{patch}", "--version-suffix", f".b{build_number}", "--type", "nodos", "--vendor", "Nodos", "--publisher-name", "nosman", "--publisher-email",
					"bot@nodos.dev"]
		if dry_run_release:
